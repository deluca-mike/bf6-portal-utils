...
export namespace ScavengerDrop {
    ...
    /**
     * Detection radius in meters for scavenging a dead player's kit bag (2 meters).
     */
    ...
    // Subscribed to OnTickStart at priority Normal (0) to evaluate drop expiration times
    // at the beginning of each tick alongside standard gameplay mechanics.
     ...
     * Subscribes to `PlayerLocations.onSphere` for a 2-meter radius (`ScavengerDrop.RADIUS`) to reactively detect scavengers.
    ...
    export function create(
        ...
        const handle = PlayerLocations.onSphere(position.x, position.y, position.z, RADIUS, (scavenger) => {
