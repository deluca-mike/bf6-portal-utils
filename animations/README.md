...
- **Tick Lifecycle Integration (`OnTickEnd` at Priority `-90`)** – Subscribes to `Events.OnTickEnd` after `Solid` (`-95`) and before `Spatial` (`-80`) and `UI.flush()` (`100`), ensuring all animation value updates (tweens, springs, decay) drive scene graph nodes and UI elements and are flushed to the engine in the exact same tick.
