...
async function getHeapAndGCMetrics(): Promise<{ heap: number; gcRuns: number; gcTime: number }> {
    forceGC();
    forceGC();
    // Yield to the event loop macrotask queue so PerformanceObserver can dispatch entries
...
describe('SolidUI Memory & Garbage Collection Profiling', () => {
    ...
    afterAll(() => {
        ...
        console.log('\n========================================================================================');
        console.log(
            `                      SOLIDUI MEMORY & GC PROFILING REPORT ${typeof global.gc === 'function' ? '(--expose-gc enabled)' : '(automatic V8 GC)'}`
        );
        console.log('========================================================================================');
    ...
    it('Scenario 1: Massive Static Graph (5,000 Signals + 5,000 Effects)', async () => {
        ...
        // Ensure per-pair cost is reasonable (< 1500 bytes per Signal+Subscriber in V8 with closures/handles)
        expect(allocatedDelta / count).toBeLessThan(1500);
    });
    ...
    it('Scenario 2: High-Frequency Steady-State Updates (25,000 mutations across 100 signals)', async () => {
        ...
        // Run mutations in batches with microtask flushes
    ...
    it('Scenario 4: Component Lifecycle Mount/Unmount Churn (500 widgets created and deleted)', async () => {
        ...
        // Mount 500 components with reactive signal bindings
        ...
        // Destroy all widgets (should clean up all effects and monkey-patched bindings)
        ...
        if (typeof global.gc === 'function') {
            expect(drift).toBeLessThan(800 * 1024);
        }
    });
