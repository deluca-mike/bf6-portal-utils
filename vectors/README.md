...
| `getDirectionFromPlayerRotation(rotation: Vector3, out?: Vector3): Vector3` | Converts a player's raw engine rotation vector (`mod.GetObjectRotation`) into a unit horizontal facing direction vector (`South = [0, 0, 1]`, `North = [0, 0, -1]`, `East = [1, 0, 0]`, `West = [-1, 0, 0]`). |
| `getHeadingFromPlayerRotation(rotation: Vector3): number` | Converts a player's raw engine rotation vector (`mod.GetObjectRotation`) into a compass heading in degrees `[0, 360)` matching the in-game HUD compass (`North = 0°`, `East = 90°`, `South = 180°`, `West = 270°`). |
| `facingToEuler(facing: Vector3, out?: Vector3): Vector3` | Converts a 3D facing direction vector into Euler rotation angles in radians (ZYX convention: `{ x: pitch, y: yaw, z: 0 }`), where North `[0, 0, -1]` is zero rotation. |
