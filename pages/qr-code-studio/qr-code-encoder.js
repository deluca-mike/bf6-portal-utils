...
(function (global) {
    ...
    class QREncoder {
        ...
        static computeReedSolomon(data, dataOffset, dataLen, eccLen, outEcc, outEccOffset) {
        ...
        static getVersionInfoBits(version) {
            let d = version << 12;
            while (d >= 1 << 12) {
        ...
        static getFormatInfoBits(eccOrIdx, mask) {
        ...
        static getSecondFormatCoord(index, size) {
         ...
         */
        static selectVersion(dataByteLen, eccIdx, maxVersion = QREncoder.MAX_QR_VERSION, logger) {
        ...
        static buildDataBytes(dataLen, version, totalDataCodewords) {
        ...
        static interleaveBlocks(version, eccIdx) {
            ...
            let dataOffset = 0;
            let eccOffset = 0;
            function processBlockGroup(startBlockIdx, numBlocks, dataPerBlock) {
            ...
            for (let i = 0; i < eccPerBlock; ++i) {
                for (let b = 0; b < totalBlocks; ++b) {
                    QREncoder._interleavedBuffer[outIdx++] =
                        QREncoder._eccBytesBuffer[QREncoder._eccBlockOffsets[b] + i];
                }
            }
            return outIdx;
        }
        static drawFinder(size, r, c, scratch) {
            for (let i = -1; i <= 7; ++i) {
                for (let j = -1; j <= 7; ++j) {
                    const nr = r + i;
                    const nc = c + j;
        ...
        static drawFinders(size, scratch) {
        ...
        static drawAlignments(size, version, scratch) {
            if (version < 2) return;
        ...
        static drawTimingPatterns(size, scratch) {
        ...
        static reserveFormatAndVersionInfo(size, version, scratch) {
            for (let i = 0; i <= 8; ++i) {
                scratch[8 * size + i] |= 1;
                scratch[i * size + 8] |= 1;
            }
        ...
        static placeDataCodewords(size, totalCodewords, scratch) {
        ...
        static getCellBit(r, c, m, size, scratch) {
        ...
        static calculatePenalty(m, size, scratch) {
            let penalty = 0;
        ...
        static evaluateBestMask(size, eccIdx, scratch) {
            let bestMask = 0;
        ...
        static applyMask(size, mask, scratch) {
            for (let r = 0; r < size; ++r) {
        ...
        static writeFormatAndVersionInfo(size, eccIdx, mask, version, scratch) {
         ...
         */
        static encodeToBuffer(data, eccOrIdx, scratchBuffer, maxVersion = QREncoder.MAX_QR_VERSION, logger) {
         ...
         */
        static encode(data, ecc = QREncoder.ECC.Medium, options) {
         ...
         */
        static encodeToAscii(data, options) {
            ...
            const fullGrid = Array.from({ length: totalSize }, (_, r) =>
         ...
         */
        static computeVerticalSpan(scratch, r, c, w, N) {
            let h = 1;
            while (r + h < N) {
         ...
         */
        static packRectangles(scratch, N, rectBuffer) {
            let rectCount = 0;
    ...
    QREncoder.MAX_QR_VERSION = 40;
    /** Default quiet zone margin in module units. */
    QREncoder.DEFAULT_MARGIN = 4;
    /** Minimum quiet zone margin in module units. */
    QREncoder.MIN_QUIET_ZONE = 4;
    QREncoder.GF_EXP = new Uint8Array(512);
    QREncoder.GF_LOG = new Uint8Array(256);
    QREncoder.GENERATOR_POLYS = new Array(31);
    (() => {
        ...
        let currentGen = new Uint8Array([1]);
        for (let i = 0; i < 30; ++i) {
    ...
    QREncoder._rsMsgBuffer = new Uint8Array(256);
    QREncoder._utf8Buffer = new Uint8Array(2953);
    QREncoder._dataCodewordsBuffer = new Uint8Array(3706);
    QREncoder._interleavedBuffer = new Uint8Array(3706);
    QREncoder._eccBytesBuffer = new Uint8Array(3706);
    QREncoder._blockOffsets = new Uint16Array(128);
    QREncoder._blockLens = new Uint16Array(128);
    QREncoder._eccBlockOffsets = new Uint16Array(128);
    QREncoder._versionResult = { version: 0, totalDataCodewords: 0 };
    /**
     ...
     */
    QREncoder.ECC_TABLE = Object.freeze([
        // V1
        [
            [7, 1, 19, 0, 0],
            [10, 1, 16, 0, 0],
    ...
    QREncoder.FORMAT_COORDS_TOP_LEFT = Object.freeze([
        [0, 8],
        [1, 8],
        [2, 8],
        [3, 8],
    ...
    class UIQRCodeExtended {
        static get _rectBuffer() {
            return _rectBuffer;
        }
        static get _scratchBuffer() {
