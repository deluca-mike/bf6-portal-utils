                ...
                run: `
                    (() => {
                        ...
                        for (let q = 0; q < ${queries}; ++q) {
                            ...
                            // 1. Sphere query
                            totalFound += PlayerLocations.findPlayersInSphere(cx, cy, cz, radius);

                            // 2. Cylinder query
                            totalFound += PlayerLocations.findPlayersInCylinder(cx, cz, radius, 0, 50);

                            // 3. AABB query
                            totalFound += PlayerLocations.findPlayersInAABB(cx - 50, 0, cz - 50, cx + 50, 50, cz + 50);

                            // 4. Directional planes
                            totalFound += PlayerLocations.findPlayersAbove(20);
                        }
                ...
                run: `
                    (() => {
                        ...
                        for (let q = 0; q < ${queries}; ++q) {
                            ...
                            const closest5 = PlayerLocations.findKClosestPlayers(cx, cy, cz, 5);
                            const farthest5 = PlayerLocations.findKFarthestPlayers(cx, cy, cz, 5);
                            const highest3 = PlayerLocations.findKHighestPlayers(3);
                            const lowest3 = PlayerLocations.findKLowestPlayers(3, filterEven);
                            const closest1 = PlayerLocations.getClosestPlayerId(cx, cy, cz);
                            const highest1 = PlayerLocations.getHighestPlayerId();
                            ...
                            checkSum +=
                                ...
                                highest3 +
                                lowest3 +
                                (closest1 !== undefined ? closest1 : 0) +
                                (highest1 !== undefined ? highest1 : 0);
                        }
            ...
            const result = server.benchmarkScenario({
                ...
                run: `
                    (() => {
                        const handles = [];
                        let flipCount = 0;
                        const onExtrema = (newPlayer, prevPlayer, newId, prevId) => {
                            ++flipCount;
                        };
