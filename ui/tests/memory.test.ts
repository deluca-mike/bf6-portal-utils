...
describe('UI Module QuickJS Runtime Memory & ARC Profiling (BF6 Portal C++ Simulation)', () => {
    ...
    beforeEach(async () => {
        ...
        server.evalCode(`
            ...
            globalThis.UIPixelArt = UIBundle.UIPixelArt;
        `);
                ...
                (() => {
                    ...
                    for (let i = 0; i < ${count}; ++i) {
                        const qr = new UIQRCode({
                            data: 'https://battlefield.portal/match/' + i,
                            scale: 1,
                            x: i * 10,
                            y: i * 10,
                        });
            ...
            run: `
                    (() => {
                        ...
                        for (let m = 0; m < ${mutations}; ++m) {
                            ...
                            qr.setColor(darkColor);
                            qr.setBgColor(lightColor);
                            qr.bgAlpha = (m % 10) / 10;
                        }
    ...
    it('Scenario 12: UIPixelArt Instantiation, Visual Mutations & Disposal', () => {
        const cycles = 100;
        const mutations = 500;
        const activeBatch = 8;
        // Sample 4x4 base64 pixel art payload
        const sampleB64 = 'AQAEAAQAAAAAAQAAAA//AAAAAP/w/wAA/wAAAAAAAQACAAIAAgABAAAABAACAAIAAgAAAAIAAgACAAIA';

        const result = server.benchmarkScenario({
            scenario: '12. PixelArt Churn & Visual Mutations',
            entities: `${cycles} cycles, ${mutations} mutations`,
            numericCount: cycles,
            unit: 'cycle',
            run: `
                    (() => {
                        ...
                        // Phase 2: Active batch under sustained 500 visual/position mutations
                        const pas = [];
                        for (let i = 0; i < ${activeBatch}; ++i) {
                            pas.push(new UIPixelArt({
                                data: '${sampleB64}',
                                width: 16,
                                height: 16,
                            }));
                        ...
                        for (let m = 0; m < ${mutations}; ++m) {
                            const pa = pas[m % ${activeBatch}];
                            pa.setColor({ r: 1, g: 0, b: 0 });
                            pa.setPosition({ x: m, y: m });
                        }
