...
function assertContainerParent(parent: MockWidget): void {
    if (!parent) {
        throw new Error('Exception: JsUndefinedValue');
    }
    if (!parent.isContainer && parent !== mockRootWidget) {
        throw new Error('Exception: Parent widget must be a UIContainer');
    }
}
...
const mockMod: Record<string, unknown> = {
    ...
    EnableUIButtonEvent: (widget: MockWidget, event: number, enabled: boolean) => {
        if (!widget || !widget.isButton) {
            throw new Error('Exception: Widget must be a UIButton to enable button events');
        }
