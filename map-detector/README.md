...
- `Error` (3) – Error messages. Includes errors when getting HQ or marker coordinates. Least verbose.
...
| Method | Description |
| --- | --- | --- |
| `setLogging(log?: (text: string, error?: unknown) => Promise<void> \| void, logLevel?: LogLevel, includeRawError?: boolean): void` | Configures logging for the MapDetector module. The map detector logs warnings when map detection fails or when maps are not available in the native enum, and errors when HQ or marker coordinate retrieval fails. Pass `undefined` (or `null`) for `log` to disable logging. Default log level is `Warning`, default `includeRawError` is `false`. See the [`Logging` module documentation](../logging/README.md). |
| `setMarkerObjectId(id: number | null): void` | Sets the spatial object ID to use as the marker for map detection instead of Team 1's HQ, or `null` to reset to default HQ1 detection. Call this at the **top of your file** (after imports), not in an event handler, so the marker ID is configured before detection runs. |
| `setCoordinates(map: MapDetector.Map, coordinates: Vectors.Vector3): void` | Sets the expected coordinates used for detecting the given map. Coordinates are rounded to nearest integers and clamped to `[-32,768, 32,767]`. Call this for **each map** where your experience uses custom spatial data that moves Team 1's HQ or marker—at the **top of your file** (after imports), not in an event handler, so coordinates are set before any detection runs. Only the **integer parts** of the coordinates are used for matching (decimals are ignored); this is sufficient because map coordinates differ significantly. |
...
2. **Coordinate Matching** – When `_markerObjectId` is defined, the detector retrieves the spatial object via `mod.GetSpatialObject(_markerObjectId)` and reads its position; otherwise, it retrieves Team 1's HQ via `mod.GetHQ(1)` and reads its position. It then compares only the **integer parts** of x, y, and z to the known arrays (decimal parts are ignored). You can override stored coordinates via `setCoordinates()` when using custom spatial layouts.
3. **Memoization (Caching)** – Because the map does not dynamically change during a single run of a Portal script, the detector caches the index after the first successful map detection. All subsequent calls to `currentMap()` or `currentNativeMap()` return the cached result instantly in `O(1)` time without polling the HQ or marker position again.
...
5. **Error Logging** – When map detection fails or coordinate retrieval encounters errors, warnings and errors are logged using the configured logger.

The detection is extremely fast, allocation-free, and requires no additional setup for default spatial data; for custom layouts, call `setCoordinates()` or `setMarkerObjectId()` at the top of your file.
