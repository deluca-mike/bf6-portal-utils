...
describe('PortalGadget Module Tests', () => {
    ...
    it('should calculate laser ray and resolve onHit when calling getLaserTarget', async () => {
        ...
        const raycastSpy = vi.spyOn(Raycast, 'cast').mockImplementation((start, end, callback) => {
            callback?.(true, { x: 10, y: 20, z: 30 }, { x: 0, y: 1, z: 0 });
            return null;
        });
    ...
    it('should resolve to undefined when laser ray misses', async () => {
        ...
        vi.spyOn(Raycast, 'cast').mockImplementation((start, end, callback) => {
            callback?.(false);
            return null;
        });
    ...
    it('should share/cache getTarget promise across handlers on same event', async () => {
        ...
        const raycastSpy = vi.spyOn(Raycast, 'cast').mockImplementation((start, end, callback) => {
            callback?.(true, { x: 5, y: 5, z: 5 }, { x: 0, y: 1, z: 0 });
            return null;
        });

        let targetPromise1: Promise<Vectors.Vector3 | undefined> | null = null;
        let targetPromise2: Promise<Vectors.Vector3 | undefined> | null = null;
