...
export namespace PortalGadget {
    ...
    const _scratch1: Vectors.Vector3 = { x: 0, y: 0, z: 0 };
    const _scratch2: Vectors.Vector3 = { x: 0, y: 0, z: 0 };
    ...
    export type Handler = (
        player: mod.Player,
        isZooming: boolean,
        getTarget: () => Promise<Vectors.Vector3 | undefined>
    ) => void;
    ...
    function _castLaserRay(
        eye: Vectors.Vector3,
        facing: Vectors.Vector3,
        isZooming: boolean
    ): Promise<Vectors.Vector3 | undefined> {
        return new Promise((resolve) => {
            ...
            Vectors.transformLocalOffset(eye, facing, offset, _scratch1);
            Vectors.rotateYawPitch(facing, yaw, pitch, _scratch2);
            Vectors.addScaled(_scratch1, _scratch2, RAYCAST_DISTANCE, _scratch2);

            try {
                Raycast.cast(_scratch1, _scratch2, (hit, hitPoint) => {
                    resolve(hit && hitPoint ? hitPoint : undefined);
                });
            } catch (error: unknown) {
    ...
    function _dispatchHandlers(handlers: Handler[], player: mod.Player): void {
        ...
        const eye = Vectors.toVector3(mod.GetSoldierState(player, mod.SoldierStateVector.EyePosition));
        const facing = Vectors.toVector3(mod.GetSoldierState(player, mod.SoldierStateVector.GetFacingDirection));
        const isZooming = mod.GetSoldierState(player, mod.SoldierStateBool.IsZooming);

        let cachedPromise: Promise<Vectors.Vector3 | undefined> | null = null;

        const getTarget = () => {
            if (cachedPromise === null) {
                cachedPromise = _castLaserRay(eye, facing, isZooming);
            }
     ...
     */
    export async function getLaserTarget(player: mod.Player): Promise<Vectors.Vector3 | undefined> {
        ...
        return _castLaserRay(Vectors.toVector3(eyeVec, _scratch1), Vectors.toVector3(faceVec, _scratch2), isZooming);
    }
}
