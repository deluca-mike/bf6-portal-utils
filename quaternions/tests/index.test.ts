...
describe('Quaternions Module Tests', () => {
    ...
    describe('2. Length, Normalization, Conjugation, Dot, and Multiplication', () => {
        it('should compute dot product of two quaternions', () => {
            const a: Quaternions.Quaternion = { w: 1, x: 2, y: 3, z: 4 };
            const b: Quaternions.Quaternion = { w: 5, x: 6, y: 7, z: 8 };
            expect(Quaternions.dot(a, b)).toBe(70);

            const q1: Quaternions.Quaternion = { w: 1, x: 0, y: 0, z: 0 };
            const q2: Quaternions.Quaternion = { w: 0, x: 1, y: 0, z: 0 };
            expect(Quaternions.dot(q1, q2)).toBe(0);
            expect(Quaternions.dot(q1, q1)).toBe(1);
        });
    ...
    describe('4. Spherical Linear Interpolation (SLERP)', () => {
        ...
        it('should compute correct pure yaw rotation from engine player rotation vectors with fromPlayerRotation', () => {
            const southVector = { x: 0, y: 0, z: 1 };
            const rotated = { x: 0, y: 0, z: 0 };

            // 1. South (rot = (0, 0, 0)) -> Facing South [0, 0, 1]
            const qSouth = Quaternions.fromPlayerRotation({ x: 0, y: 0, z: 0 });
            Quaternions.rotateVector(southVector, qSouth, rotated);
            expect(rotated.x).toBeCloseTo(0);
            expect(rotated.y).toBeCloseTo(0);
            expect(rotated.z).toBeCloseTo(1);

            // 2. East of South (rot = (0, 0.04, 0)) -> +0.04 rad East (+X)
            const qEastOfSouth = Quaternions.fromPlayerRotation({ x: 0, y: 0.04, z: 0 });
            Quaternions.rotateVector(southVector, qEastOfSouth, rotated);
            expect(rotated.x).toBeCloseTo(Math.sin(0.04), 4);
            expect(rotated.z).toBeCloseTo(Math.cos(0.04), 4);

            // 3. South of East (rot = (0, 1.53, 0)) -> ~+87.7 deg
            const qSouthOfEast = Quaternions.fromPlayerRotation({ x: 0, y: 1.53, z: 0 });
            Quaternions.rotateVector(southVector, qSouthOfEast, rotated);
            expect(rotated.x).toBeCloseTo(Math.sin(1.53), 4);
            expect(rotated.z).toBeCloseTo(Math.cos(1.53), 4);

            // 4. North of East (rot = (3.14159, 1.53, 3.14159)) -> ~+92.3 deg (Pi - 1.53)
            const qNorthOfEast = Quaternions.fromPlayerRotation({ x: Math.PI, y: 1.53, z: Math.PI });
            Quaternions.rotateVector(southVector, qNorthOfEast, rotated);
            expect(rotated.x).toBeCloseTo(Math.sin(Math.PI - 1.53), 4);
            expect(rotated.z).toBeCloseTo(Math.cos(Math.PI - 1.53), 4);

            // 5. East of North (rot = (3.14159, 0.04, 3.14159)) -> ~+177.7 deg (Pi - 0.04)
            const qEastOfNorth = Quaternions.fromPlayerRotation({ x: Math.PI, y: 0.04, z: Math.PI });
            Quaternions.rotateVector(southVector, qEastOfNorth, rotated);
            expect(rotated.x).toBeCloseTo(Math.sin(Math.PI - 0.04), 4);
            expect(rotated.z).toBeCloseTo(Math.cos(Math.PI - 0.04), 4);

            // 6. West of North (rot = (3.14159, -0.04, 3.14159)) -> ~-177.7 deg (-Pi - (-0.04))
            const qWestOfNorth = Quaternions.fromPlayerRotation({ x: Math.PI, y: -0.04, z: Math.PI });
            Quaternions.rotateVector(southVector, qWestOfNorth, rotated);
            expect(rotated.x).toBeCloseTo(Math.sin(-Math.PI + 0.04), 4);
            expect(rotated.z).toBeCloseTo(Math.cos(-Math.PI + 0.04), 4);

            // 7. North of West (rot = (3.14159, -1.53, 3.14159)) -> ~-92.3 deg (-Pi - (-1.53))
            const qNorthOfWest = Quaternions.fromPlayerRotation({ x: Math.PI, y: -1.53, z: Math.PI });
            Quaternions.rotateVector(southVector, qNorthOfWest, rotated);
            expect(rotated.x).toBeCloseTo(Math.sin(-Math.PI + 1.53), 4);
            expect(rotated.z).toBeCloseTo(Math.cos(-Math.PI + 1.53), 4);

            // 8. South of West (rot = (0, -1.53, 0)) -> ~-87.7 deg
            const qSouthOfWest = Quaternions.fromPlayerRotation({ x: 0, y: -1.53, z: 0 });
            Quaternions.rotateVector(southVector, qSouthOfWest, rotated);
            expect(rotated.x).toBeCloseTo(Math.sin(-1.53), 4);
            expect(rotated.z).toBeCloseTo(Math.cos(-1.53), 4);

            // 9. Out parameter reuse
            const out: Quaternions.Quaternion = { w: 0, x: 0, y: 0, z: 0 };
            const qOut = Quaternions.fromPlayerRotation({ x: 0, y: 1.2, z: 0 }, out);
            expect(qOut).toBe(out);
            expect(qOut.w).toBeCloseTo(Math.cos(0.6));
            expect(qOut.y).toBeCloseTo(Math.sin(0.6));
        });
    });
