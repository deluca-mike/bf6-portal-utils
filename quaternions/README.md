...
##### `Quaternions.dot(a: Quaternion, b: Quaternion): number`

Computes the 4D dot product between two quaternions ($a.w \cdot b.w + a.x \cdot b.x + a.y \cdot b.y + a.z \cdot b.z$).
...
##### `Quaternions.fromAxisAngle(axis: Vector3, angleRad: number, out?: Quaternion): Quaternion`

Constructs a rotation quaternion from an arbitrary 3D axis and angle in radians.

##### `Quaternions.fromLookRotation(forward: Vector3, up?: Vector3, out?: Quaternion): Quaternion`
...
##### `Quaternions.fromEuler(pitchRad: number, yawRad: number, rollRad: number, out?: Quaternion): Quaternion`

Constructs a rotation quaternion from Euler angles in radians using ZYX order.

##### `Quaternions.fromFacing(facing: Vector3, out?: Quaternion): Quaternion`

Constructs an orientation quaternion from a 3D facing direction vector (where facing due North `[0, 0, -1]` is zero rotation / identity). Pitch is elevation relative to the horizon and Yaw is horizontal compass bearing. Safely normalizes the input vector.

##### `Quaternions.fromPlayerRotation(rotation: Vector3, out?: Quaternion): Quaternion`

Constructs a pure horizontal yaw rotation quaternion from a player's raw engine rotation vector obtained via `mod.GetObjectRotation(player)`. Unwraps Frostbite's northern-hemisphere Euler representation into continuous horizontal yaw in radians with zero heap allocations.
