...
describe('Logger Module Tests', () => {
    ...
    describe('Dynamic Row Logging and Batching', () => {
        it('should batch log messages and flush on Events.OnTickEnd', () => {
            const logger = new Logger(mockPlayer, {
            ...
            // Log 4th message before flush - will recycle top row when flushed
        ...
        it('should support manual immediate flush()', () => {
            const logger = new Logger(mockPlayer);

            logger.log('Manual flush test');
            expect(() => logger.flush()).not.toThrow();

            logger.delete();
        });

        it('should skip earlier messages on large queue overflow in a single tick', () => {
            const logger = new Logger(mockPlayer, {
                height: 88, // 3 max rows
            });

            // Log 50 messages in one tick
            for (let i = 0; i < 50; ++i) {
                logger.log(`Flood ${i}`);
            }
        ...
        it('should discard pending logs if clear() is called before flush', () => {
            const logger = new Logger(mockPlayer);

            logger.log('Discarded message');
            logger.clear();
    ...
    describe('Static Row Logging', () => {
        it('should log to specific row indices and coalesce multiple writes to the same row', () => {
            const logger = new Logger(mockPlayer, {
                height: 100, // 4 max rows: indices 0..3
                staticRows: true,
            });

            // Overwrite row 0 multiple times before flush
            logger.log('Row 0 text A', 0);
            logger.log('Row 0 text B', 0);
            logger.log('Row 0 text final', 0);
