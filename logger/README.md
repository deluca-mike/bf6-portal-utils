...
| `clear(): Logger` | Deletes every rendered row/container and clears any pending unflushed logs. |
| `delete(): void` | Calls `clear()` and removes the window container entirely—use when unloading the logger. |
| `log(text: string, rowIndex?: number): Logger` | Core logging entry point. In static mode, writes to `rowIndex` (default `0`). In dynamic mode, appends to the log queue. All writes are batched and rendered at `Events.OnTickEnd` (priority 90). |
| `flush(): Logger` | Flushes all pending batched log entries immediately (normally executed automatically at `Events.OnTickEnd`). |

Internals worth noting when debugging:

- **Batched Frame Rendering (`OnTickEnd` @ 90)**: Logger subscribes to `Events.OnTickEnd` at priority 90, directly before `UI.flush()` (100). All text tokenization and UIContainer manipulations are deferred to this single end-of-frame step.
- **Static Overwrite Coalescing & Dynamic Overflow Skipping**: In static mode, multiple writes to the same row in a single tick only parse and render the final state. In dynamic mode, if more lines are queued than `maxRows`, overflow entries that would scroll off-screen are skipped without rendering.
...
| `anchor` | `UI.Anchor` | `UI.Anchor.TopLeft` | Determines how `x`/`y` offsets are interpreted. |
...
| `bgFill` | `UI.BgFill` | `UI.BgFill.Blur` | Background fill style for the container. |
