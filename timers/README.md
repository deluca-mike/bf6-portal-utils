...
2. **Ongoing Tick Evaluation** – The namespace registers a single callback to the engine's global tick loop (`Events.OnTickStart`). To resolve load-time circular import loops between the `Timers` and `Events` modules, this subscription is established lazily when the first timer is created. On each game tick:
