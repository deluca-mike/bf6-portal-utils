...
The **`currentTick`** value is **not** an engine tick API; it is incremented once per `Events.OnTickStart` subscription callback. `deferTicks` and per-tick safety limits are defined relative to that logical tick.
