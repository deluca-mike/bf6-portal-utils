...
4. Call `Spatial.sync()` in your ongoing tick loop (or throttled cadence) to flush dirty transforms to the engine. The scene graph automatically advances physics kinematics, orbiters, and look-at/follow trackers in memory via `Events.OnTickEnd` (priority `-80`), ensuring all animation mutations (`-90`) are incorporated before transforms are evaluated and flushed.
