...
export namespace Spatial {
    ...
    const _queryRotScratch: Quaternion = { w: NaN, x: 0, y: 0, z: 0 };
    ...
    function _allocateSlot(): number {
        ...
        _setQuaternionIdentityInArray(_localRot, slot);
        _setVectorInArray(_localScale, slot, 1);
        _setVectorInArray(_pivotOffset, slot, 0);
        _setVectorInArray(_worldPos, slot, 0);
        _setQuaternionIdentityInArray(_worldRot, slot);
        _setVectorInArray(_worldScale, slot, 1);
        _setVectorInArray(_lastSyncedPos, slot, 0);
        _setQuaternionIdentityInArray(_lastSyncedRot, slot);
    ...
    function _vectorEqualsArray(vector: Vector3, array: Float32Array, idx: number): boolean {
        const idx3 = idx * 3;
        return vector.x === array[idx3] && vector.y === array[idx3 + 1] && vector.z === array[idx3 + 2];
    }
    ...
    function _setQuaternionIdentityInArray(targetArray: Float32Array, tIdx: number): void {
        const tIdx4 = tIdx * 4;
        targetArray[tIdx4] = 1;
        targetArray[tIdx4 + 1] = 0;
        targetArray[tIdx4 + 2] = 0;
        targetArray[tIdx4 + 3] = 0;
    }
    ...
    function _initSlotTransforms(slot: number, parentSlot: number, params?: NodeParams): void {
        ...
        if (rot) {
            if ('w' in rot) {
                ...
                Quaternions.fromEuler(rot.x, rot.y, rot.z, _rotateLocalRot);
    ...
    function _syncNode(slot: number): void {
        ...
        if (obj && mod.IsValid(obj)) {
            ...
            if (!shouldSync) {
                const sIdx3 = slot * 3;
                const dx = renderPos.x - _lastSyncedPos[sIdx3];
                const dy = renderPos.y - _lastSyncedPos[sIdx3 + 1];
                const dz = renderPos.z - _lastSyncedPos[sIdx3 + 2];
                ...
                if (posThreshSq <= 0 ? distSq > 0 : distSq >= posThreshSq) {
                    ...
                    const sIdx4 = slot * 4;
                    const w = _lastSyncedRot[sIdx4];
                    const x = _lastSyncedRot[sIdx4 + 1];
                    const y = _lastSyncedRot[sIdx4 + 2];
                    const z = _lastSyncedRot[sIdx4 + 3];

                    const lenSq1 = Quaternions.lengthSquared(_renderRot);
                    const lenSq2 = w * w + x * x + y * y + z * z;

                    if (lenSq1 >= 1e-6 || lenSq2 >= 1e-6) {
                        const dot = Math.abs(_renderRot.w * w + _renderRot.x * x + _renderRot.y * y + _renderRot.z * z);
            ...
            if (shouldSync) {
                ...
                try {
                    ...
                    _copyVectorIntoArray(_lastSyncedPos, slot, renderPos);
                    _copyQuaternionIntoArray(_lastSyncedRot, slot, _renderRot);
    ...
    function _getTargetRotation(
        ...
        switch (targetType) {
            ...
            case TargetType.QueryableObject: {
                ...
                _queryRotScratch.w = NaN;
                _queryRotScratch.x = 0;
                _queryRotScratch.y = 0;
                _queryRotScratch.z = 0;

                const rot = q.getRotation(_queryRotScratch);
            ...
            case TargetType.Vehicle: {
                ...
                return Quaternions.fromFacing(_followFacing, out);
            }
            case TargetType.TrackableObject: {
                if (!mod.IsValid(target)) return undefined;
                ...
                return Quaternions.fromEuler(_followEuler.x, _followEuler.y, _followEuler.z, out);
            }
            default:
                return undefined;
        }
    ...
    function _stepKinematics(slot: number, dtSeconds: number): void {
        ...
        if (kin.angularVelocity) {
            ...
            if (angle > 0) {
                Quaternions.fromAxisAngle(kin.angularVelocity, angle, _kinematicsAngularRot);
    ...
    function _stepOrbit(slot: number, dtSeconds: number): void {
        ...
        Quaternions.fromAxisAngle(axis, orb.currentAngleRad, _orbitRot);
        ...
        if (orb.faceTangent) {
            ...
            if (Vectors.lengthSquared(_orbitTangentCross) > 0) {
                ...
                _copyQuaternionIntoArray(_localRot, slot, Quaternions.fromEuler(0, yaw, 0, _orbitRot));
            }
        }

        if (orb.selfSpinSpeedRadPerSec) {
            Quaternions.fromAxisAngle(_UP_AXIS, orb.selfSpinSpeedRadPerSec * dtSeconds, _orbitSpinRot);
    ...
    function _stepFollow(slot: number, dtMs: number): void {
        ...
        if (hasOrientation && follow.yawOnly) {
            ...
            Quaternions.fromEuler(0, _followEuler.y, 0, _followTargetRot);
        }
        ...
        if (typeof smoothing === 'function') {
            ...
            if (Vectors.distanceSquared(_followLerpedPos, _followTargetWithOffset) < 1e-8) {
                Vectors.copy(_followLerpedPos, _followTargetWithOffset);
            }
        }
    ...
    export function setLocalPosition(id: SpatialNodeID, pos: Vector3): void {
        ...
        if (_vectorEqualsArray(pos, _localPos, slot)) return;
    ...
    export function setLocalRotation(id: SpatialNodeID, rot: Quaternion): void {
        ...
        const sIdx4 = slot * 4;
        const w = _localRot[sIdx4];
        const x = _localRot[sIdx4 + 1];
        const y = _localRot[sIdx4 + 2];
        const z = _localRot[sIdx4 + 3];
    ...
    export function setLocalRotationEuler(id: SpatialNodeID, euler: Vector3): void {
        ...
        Quaternions.fromEuler(euler.x, euler.y, euler.z, _rotateLocalRot);
    ...
    export function setLocalScale(id: SpatialNodeID, scale: Vector3 | number): void {
        ...
        if (typeof scale === 'number') {
            ...
            if (_vectorEqualsArray(scale, _localScale, slot)) return;
    ...
    export function setWorldRotationEuler(id: SpatialNodeID, worldEuler: Vector3): void {
        Quaternions.fromEuler(worldEuler.x, worldEuler.y, worldEuler.z, _worldSetEulerRot);
    ...
    export function setPivotOffset(id: SpatialNodeID, offset?: Vector3 | null): void {
        ...
        if (offset) {
            if (_hasFlag(slot, FLAG_HAS_PIVOT) && _vectorEqualsArray(offset, _pivotOffset, slot)) return;
    ...
    export function rotateAroundAxis(id: SpatialNodeID, axis: Vector3, angleRad: number, pivotCenter?: Vector3): void {
        ...
        Quaternions.fromAxisAngle(axis, angleRad, _rotateAroundAxisRot);
    ...
    export function lookAt(id: SpatialNodeID, targetWorld: Vector3, upAxis?: Vector3): void {
        ...
        Quaternions.fromLookRotation(_lookAtDeltaPos, upAxis ?? _UP_AXIS, _lookAtRot);
