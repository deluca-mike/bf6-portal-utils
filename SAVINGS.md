...
The `Physics` module was engineered from the ground up as a high-performance **Structure of Arrays (SoA)** dynamic simulation layer that integrates with `Spatial` scene graph nodes.

### Memory & Allocation Breakdown

Allocations are fixed and pre-allocated upfront across `MAX_BODIES = 64` slots:

- `_posX`, `_posY`, `_posZ` (`3x Float32Array`): 12 bytes/slot (768 B)
- `_velX`, `_velY`, `_velZ` (`3x Float32Array`): 12 bytes/slot (768 B)
- `_normX`, `_normY`, `_normZ` (`3x Float32Array`): 12 bytes/slot (768 B)
- `_gravX`, `_gravY`, `_gravZ` (`3x Float32Array`): 12 bytes/slot (768 B)
- Physical parameters (`9x Float32Array`: mass, invMass, radius, restitution, friction, staticFriction, rollingFriction, linearDamping, angularDamping): 36 bytes/slot (2,304 B)
- Bitflags and handles (`_spatialNodeIds` `Int32Array`, `_generations` `Uint16Array`, `_freeList` `Int8Array`, `_flags` `Uint8Array`, `_flags2` `Uint8Array`, `_lookaheadTicks` `Uint8Array`): 10 bytes/slot (640 B)
- **Marginal Cost per Body Slot**: **84 bytes**
- **Total Resident Pool Memory (64 Bodies)**: **~6.0 KB**

### Steady-State & GC Performance

- **Zero-Allocation Simulation Loop**: Advances 64 dynamic physical bodies through airborne ballistics, rolling surface friction, resting state transitions, and in-memory multi-body collisions with **0 bytes of heap garbage**.
- **Multi-Tick Lookahead Optimization**: Airborne bodies cast conservative lookahead rays that cache clear corridors for 3–5 ticks upon miss, reducing raycast queue pressure by up to 80%.
- **Zero Raycasts for Sleeping Bodies & Object Collisions**: Objects at rest consume 0 raycasts and 0 transform updates. Mutual collisions between dynamic bodies are resolved analytically in memory with zero native engine raycasts.
...
| **`Raycast`** | Ring Buffer Queue + Dedicated Worker Slots | ~160 B / ray + $O(N)$ fuzzy map search | **~23.1 KB pool** (512 queue, 101 workers) | **0 B / raycast** (100% eliminated) | 4-tier priority queues (`Critical` > `Physics` > `Standard` > `Ambient`); parallel drain. |
| **`Physics`** | Structure of Arrays (`MAX_BODIES = 64`) | Class instances (~400 B / body) | **84 B / slot** (~6.0 KB pool) | **0 B / tick** (100% eliminated) | Multi-tick conservative lookahead; in-memory sphere-sphere resolution; 0 raycast sleep states. |
