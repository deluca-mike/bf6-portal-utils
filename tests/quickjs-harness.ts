...
export async function createQuickJSServerContext(options?: {
    ...
    // Host-side mock widget and player storage (ZERO QuickJS heap footprint for mock storage)
    const hostWidgets = new Map<string, Record<string, unknown>>();
    let nextWidgetId = 1;
    const hostPlayers = new Map<number, { id: number; connected: boolean; x: number; y: number; z: number }>();
    ...
    registerHostFunction('IsValid', (objH) =>
        context.typeof(objH) === 'object' || context.typeof(objH) === 'number' ? context.true : context.false
    );

    registerHostFunction('SpawnObject', () => {
        const obj = context.newObject();
        const idH = context.newNumber(nextWidgetId++);
        context.setProp(obj, '_id', idH);
        idH.dispose();
        return obj;
    });

    registerHostFunction('SetObjectTransform', () => context.undefined);

    registerHostFunction('CreateTransform', (posH, rotH) => {
        const tr = context.newObject();
        if (posH) context.setProp(tr, 'position', posH);
        if (rotH) context.setProp(tr, 'rotation', rotH);
        return tr;
    });

    registerHostFunction('GetObjectPosition', (objH) => {
        let x = 0;
        let y = 0;
        let z = 0;
        if (objH && context.typeof(objH) === 'object') {
            const idH = context.getProp(objH, '_id');
            if (context.typeof(idH) === 'number') {
                const id = context.getNumber(idH);
                const p = hostPlayers.get(id);
                if (p) {
                    x = p.x;
                    y = p.y;
                    z = p.z;
        ...
        const vec = context.newObject();
        const xH = context.newNumber(x);
        const yH = context.newNumber(y);
        const zH = context.newNumber(z);
        context.setProp(vec, 'x', xH);
        context.setProp(vec, 'y', yH);
        context.setProp(vec, 'z', zH);
        xH.dispose();
    ...
    registerHostFunction('GetObjectRotation', () => {
        const vec = context.newObject();
        const x = context.newNumber(0);
        const y = context.newNumber(0);
        const z = context.newNumber(0);
        context.setProp(vec, 'x', x);
        context.setProp(vec, 'y', y);
        context.setProp(vec, 'z', z);
        x.dispose();
        y.dispose();
        z.dispose();
        return vec;
    ...
    registerHostFunction('GetSoldierState', (_playerH, typeH) => {
        const vec = context.newObject();
        const typeNum = context.typeof(typeH) === 'number' ? context.getNumber(typeH) : 0;
        const x = context.newNumber(0);
        const y = context.newNumber(0);
        const z = context.newNumber(typeNum === 2 ? 1 : 0);
        context.setProp(vec, 'x', x);
        context.setProp(vec, 'y', y);
        context.setProp(vec, 'z', z);
        x.dispose();
    ...
    registerHostFunction('GetVehicleState', (_vehicleH, typeH) => {
        const vec = context.newObject();
        const typeNum = context.typeof(typeH) === 'number' ? context.getNumber(typeH) : 0;
        const x = context.newNumber(0);
        const y = context.newNumber(0);
    ...
    registerHostFunction('AllPlayers', () => {
        const arr = context.newArray();
        let idx = 0;
        for (const p of hostPlayers.values()) {
            if (p.connected) {
                const pObj = context.newObject();
                const idH = context.newNumber(p.id);
                context.setProp(pObj, '_id', idH);
                idH.dispose();
                context.setProp(arr, idx++, pObj);
                pObj.dispose();
            }
        }
        return arr;
    ...
    registerHostFunction('CountOf', (arrH) => {
        if (arrH && context.typeof(arrH) === 'object') {
            const lenH = context.getProp(arrH, 'length');
            if (context.typeof(lenH) === 'number') {
                return lenH;
            }
            lenH.dispose();
        }
    ...
    registerHostFunction('ValueInArray', (arrH, idxH) => {
        if (arrH && context.typeof(arrH) === 'object' && context.typeof(idxH) === 'number') {
            const idx = context.getNumber(idxH);
            return context.getProp(arrH, idx);
        }
        return context.undefined;
    });

    registerHostFunction('GetPlayer', (idH) => {
        const id = context.typeof(idH) === 'number' ? context.getNumber(idH) : -1;
        const p = hostPlayers.get(id);
        if (!p || !p.connected) {
            return context.undefined;
        }
        const pObj = context.newObject();
        const idNum = context.newNumber(p.id);
        context.setProp(pObj, '_id', idNum);
        idNum.dispose();
        return pObj;
    });

    registerHostFunction('__setPlayer', (idH, xH, yH, zH, connH) => {
        const id = context.getNumber(idH);
        const x = context.getNumber(xH);
        const y = context.getNumber(yH);
        const z = context.getNumber(zH);
        const connected = connH && context.typeof(connH) !== 'undefined' ? context.dump(connH) !== false : true;
        hostPlayers.set(id, { id, connected, x, y, z });
        return context.undefined;
    });

    registerHostFunction('__initPlayers', (countH) => {
        const count = context.typeof(countH) === 'number' ? context.getNumber(countH) : 100;
        hostPlayers.clear();
    ...
    registerHostFunction('__clearPlayers', () => {
        hostPlayers.clear();
        return context.undefined;
    });
    ...
    setEnum('SoldierStateVector', { GetPosition: 1, GetFacingDirection: 2 });
    setEnum('VehicleStateVector', { VehiclePosition: 1, FacingDirection: 2 });
    setEnum('RuntimeSpawn_Common', { Sphere_01: 1, FiringRange_Floor_01: 2, Target_01: 3 });
