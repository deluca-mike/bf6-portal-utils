...
/**
 * Rectangular region in logical grid coordinates.
 */
...
/**
 * Supported palette quantization algorithms.
 */
export type PaletteAlgorithm = 'median-cut' | 'most-frequent' | 'user-defined';

/**
 * Options for the pixel art encoder.
 */
export interface PixelArtEncoderOptions {
    /** Maximum number of colors in the output palette (1..256). Default is 32. */
    maxColors?: number;
    /** Quantization algorithm to use. Default is 'median-cut'. */
    paletteAlgorithm?: PaletteAlgorithm;
    /** Custom user-defined palette (used when paletteAlgorithm is 'user-defined' or when supplied). */
    customPalette?: (RgbaColor | string)[];
    /** Whether to retain/support alpha transparency from the original image. Default is true. */
    supportOpacity?: boolean;
    /** Whether to enforce monochrome output (with alpha levels when supportOpacity is true). Default is false. */
    monochrome?: boolean;
    /** Scale factor (e.g. 1.0 = original size, 0.5 = 50% size, etc.). Default is 1.0. */
    targetScale?: number;
    /**
     * Alpha threshold (1..255 or 0.0..1.0) above or equal to which pixels are considered solid (or preserved)
     * and below which are discarded as transparent cutouts. Default is 128 when supportOpacity is false, 1 when true.
     */
    alphaThreshold?: number;
}

/**
 * Output payload from the encoder.
 */
export interface PixelArtEncodedData {
    width: number;
    height: number;
    hasOpacity: boolean;
    is16BitCoords: boolean;
    isMonochrome: boolean;
    palette: RgbaColor[];
    rectangles: EncodedRect[];
    base64: string;
    base122: string;
    stats: {
 ...
 */
export function parseColor(input: string | RgbaColor): RgbaColor {
 ...
 */
export function resolveAlphaThreshold(threshold: number | undefined, supportOpacity: boolean): number {
 ...
 */
export function deltaEOklab(c1: RgbaColor, c2: RgbaColor): number {
 ...
 */
export function hilbert3D(x: number, y: number, z: number, bits: number = 10): number {
 ...
 */
export function sortPalettePerceptual(palette: RgbaColor[]): RgbaColor[] {
    ...
    const path: typeof items = [unvisited.splice(minLIdx, 1)[0]];
    ...
    while (improved && iterations < 50 && n > 3) {
        ...
        for (let i = 0; i < n - 2; i++) {
            for (let j = i + 2; j < n; j++) {
                ...
                const d = (l1: typeof c_i, l2: typeof c_i) => {
                    const dL = l1.L - l2.L;
                    const da = l1.a - l2.a;
                    const db = l1.b - l2.b;
 ...
 */
function getColorDistanceSq(c1: RgbaColor, c2: RgbaColor, checkAlpha: boolean): number {
    const dr = c1.r - c2.r;
    const dg = c1.g - c2.g;
    const db = c1.b - c2.b;
 ...
 */
function findNearestPaletteIndex(color: RgbaColor, palette: RgbaColor[], checkAlpha: boolean): number {
    let nearestIdx = 0;
...
/**
 * Median-cut color quantization box.
 */
interface ColorBox {
    colors: RgbaColor[];
}
 ...
 */
function splitColorBox(box: ColorBox, checkAlpha: boolean): [ColorBox, ColorBox] {
    let minR = 255,
        maxR = 0;
    let minG = 255,
        maxG = 0;
 ...
 */
function getAverageColor(colors: RgbaColor[]): RgbaColor {
...
export function quantizeMedianCut(
    ...
    const uniqueMap = new Map<number, RgbaColor>();
    ...
    const boxes: ColorBox[] = [{ colors: validPixels }];
 ...
 */
export function quantizeMostFrequent(
    pixels: RgbaColor[],
    maxColors: number,
    checkAlpha: boolean,
    alphaThreshold: number
    ...
    const freqMap = new Map<number, { color: RgbaColor; count: number }>();
...
export function generatePalette(
    ...
    checkAlpha: boolean,
    alphaThreshold: number,
    algorithm: PaletteAlgorithm = 'median-cut',
    customPalette?: (RgbaColor | string)[]
): RgbaColor[] {
 ...
 */
export function quantizeAlphaMedianCut(pixels: RgbaColor[], maxAlphas: number, alphaThreshold: number): RgbaColor[] {
 ...
 * @returns Array of RGBA colors with r=255, g=255, b=255 and discrete alpha levels sorted ascending.
 */
export function quantizeAlphaMostFrequent(pixels: RgbaColor[], maxAlphas: number, alphaThreshold: number): RgbaColor[] {
    const counts = new Map<number, number>();
 ...
 */
export function generateAlphaPalette(
    pixels: RgbaColor[],
    maxAlphas: number,
    alphaThreshold: number,
    algorithm: PaletteAlgorithm = 'median-cut',
    customPalette?: (RgbaColor | string | number)[]
): RgbaColor[] {
    if (algorithm === 'user-defined' || (customPalette && customPalette.length > 0)) {
        if (!customPalette || customPalette.length === 0) return [{ r: 255, g: 255, b: 255, a: 255 }];
 ...
 */
export const quantizeAlphaPalette = generateAlphaPalette;
 ...
 */
export function findNearestAlphaIndex(alpha: number, palette: RgbaColor[]): number {
    let bestIdx = 0;
...
export function resampleImageData(
    ...
    const output: RgbaColor[] = new Array(dstWidth * dstHeight);
 ...
 */
function canDrawOpaqueRect(
    targetGrid: Int16Array,
    currentGrid: Int16Array,
...
function findMaxOpaqueSpanH(
    ...
    while (y0 + h < gridHeight) {
        if (!canDrawOpaqueRect(targetGrid, currentGrid, alphaGrid, gridWidth, gridHeight, x0, y0, w, h + 1, colorIdx)) {
            break;
        }
 ...
 */
function decomposeOpaqueLayers(
    targetGrid: Int16Array,
    currentGrid: Int16Array,
    alphaGrid: Uint8Array,
    gridWidth: number,
 ...
 */
function decomposeTranslucentLayers(
    targetGrid: Int16Array,
        ...
        for (let x = 0; x < gridWidth; ++x) {
            ...
            while (y + h < gridHeight) {
                ...
                for (let dx = 0; dx < w; ++dx) {
                    ...
                    if (visitedGrid[testIdx] !== 0 || targetGrid[testIdx] !== colorIdx || alphaGrid[testIdx] !== a) {
                        rowValid = false;
                        break;
                    }
                }
...
/****** Binary Serialization & Base64 Encoding ******/

/**
 * Deserialized pixel art payload matching the parameters of serializePixelArt.
 */
export interface DeserializedPixelArt {
    width: number;
    height: number;
    hasOpacity: boolean;
    is16BitCoords: boolean;
...
export const B122_ILLEGALS = [0, 10, 13, 34, 38, 92]; // \0, \n, \r, ", &, \
export const B122_SHORTENED = 0b111; // 7
 ...
 */
export function encodeBase122(rawData: Uint8Array): string {
    let curIndex = 0;
    let curBit = 0;
    const outBytes: number[] = [];

    function get7(): number | false {
 ...
 */
export function decodeBase122(strData: string, outBuffer?: Uint8Array): Uint8Array {
 ...
 * @returns Base122 encoded string.
 */
export function bytesToBase122(bytes: Uint8Array): string {
 ...
 */
export function base122ToBytes(base122: string): Uint8Array {
 ...
 */
export function isBase122(str: string): boolean {
 ...
 */
export function deserializePixelArt(input: Uint8Array | string): DeserializedPixelArt {
    const bytes = typeof input === 'string' ? (isBase122(input) ? base122ToBytes(input) : base64ToBytes(input)) : input;
    let offset = 0;
    ...
    const rectangles: EncodedRect[] = new Array(rectCount);
    const hasPaletteIndex = !isMonochrome || hasOpacity;
 ...
 */
export function encodePixelArtFromRgba(
    pixels: RgbaColor[],
    width: number,
    height: number,
    options: PixelArtEncoderOptions = {}
): PixelArtEncodedData {
    ...
    const isAutoMonochrome1Color = options.monochrome === undefined && maxColors === 1 && algorithm !== 'user-defined';
    ...
    if (isMonochrome) {
        ...
        palette = generatePalette(pixels, maxColors, supportOpacity, alphaThreshold, algorithm, options.customPalette);
...
/**
 * Reads a PNG file from disk and encodes it into the pixel art format.
 * @param filePath - File path to PNG image.
 * @param options - Encoding options.
 * @returns Encoded pixel art data and statistics.
 */
export function encodePixelArtFromFile(filePath: string, options: PixelArtEncoderOptions = {}): PixelArtEncodedData {
    const buffer = fs.readFileSync(filePath);
    const png = PNG.sync.read(buffer);

    const targetScale = options.targetScale ?? 1.0;
    const targetWidth = Math.max(1, Math.round(png.width * targetScale));
    const targetHeight = Math.max(1, Math.round(png.height * targetScale));
    ...
    return encodePixelArtFromRgba(pixels, targetWidth, targetHeight, options);
}
 ...
 */
export type PixelArtCliOutType = 'base64' | 'base122';

/**
 * Parses CLI command-line arguments and runs the encoder.
 * @param customArgs - Optional custom arguments array (defaults to process.argv.slice(2)).
 */
export function runCli(customArgs?: string[]): void {
    const args = customArgs ?? process.argv.slice(2);

    if (args.length === 0 || args.includes('--help') || args.includes('-h')) {
        console.log(`
Usage: npx ts-node scripts/encode-pixel-art.ts <image-path.png> [options]

Options:
  --out-type <type>                  Output payload encoding: base64 | base122 (default: base64)
  --max-colors <number>              Maximum palette colors (or alpha levels in monochrome mode, 1..256, default: 32)
  --palette-algorithm <algorithm>    Quantization algorithm: median-cut | most-frequent | user-defined (default: median-cut)
  --palette <colors>                 Comma-separated list of hex or rgb colors for user-defined palette
  --monochrome                       Encode as monochrome graphic (uses compact alpha palette if opacity enabled)
  --scale <number>                   Scale multiplier (e.g. 0.5 for 50%, default: 1.0)
  --no-opacity                       Disable opacity retention (forces opaque RGB)
  --alpha-threshold <number>         Alpha threshold (1..255 or 0..1, default: 128 when no-opacity, 1 with opacity)
...
`);
    ...
    let outType: PixelArtCliOutType = 'base64';
    let maxColors = 32;
    let paletteAlgorithm: PaletteAlgorithm | undefined;
    let customPalette: string[] | undefined;
    let monochrome = false;
    let targetScale = 1.0;
    let supportOpacity = true;
    let alphaThreshold: number | undefined;
    let jsonOutput = false;

    for (let i = 0; i < args.length; ++i) {
        const arg = args[i];
        if ((arg === '--out-type' || arg === '--outType') && args[i + 1]) {
            const val = args[++i].toLowerCase();
            if (val !== 'base64' && val !== 'base122') {
                console.error(`Error: Invalid outType "${val}". Must be 'base64' or 'base122'.`);
                process.exit(1);
            }
            outType = val as PixelArtCliOutType;
        } else if (arg.startsWith('--out-type=') || arg.startsWith('--outType=')) {
            ...
            outType = val as PixelArtCliOutType;
        } else if (arg === '--max-colors' && args[i + 1]) {
            maxColors = parseInt(args[++i], 10);
        } else if ((arg === '--palette-algorithm' || arg === '--algorithm') && args[i + 1]) {
            paletteAlgorithm = args[++i] as PaletteAlgorithm;
        } else if ((arg === '--palette' || arg === '--custom-palette') && args[i + 1]) {
            customPalette = args[++i].split(',').map((s) => s.trim());
        } else if (arg === '--monochrome') {
            monochrome = true;
        } else if (arg === '--scale' && args[i + 1]) {
            targetScale = parseFloat(args[++i]);
        } else if (arg === '--no-opacity') {
            supportOpacity = false;
        } else if (arg === '--alpha-threshold' && args[i + 1]) {
            alphaThreshold = parseFloat(args[++i]);
        } else if (arg === '--json') {
            jsonOutput = true;
        } else if (!arg.startsWith('-') && !imagePath) {
            imagePath = path.resolve(arg);
        }
    }

    if (!imagePath) {
        console.error('Error: No input image file specified.');
        process.exit(1);
    }
    ...
    const result = encodePixelArtFromFile(imagePath, {
        maxColors,
        paletteAlgorithm,
        customPalette,
        monochrome,
    ...
    console.log(outType === 'base122' ? result.base122 : result.base64);
}

// Execute if run directly from command line
if (typeof process !== 'undefined' && process.argv && process.argv[1]?.endsWith('encode-pixel-art.ts')) {
    runCli();
}
